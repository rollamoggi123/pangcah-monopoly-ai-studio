...
const dataRaw = fs.readFileSync('topic9_data_mapped.json', 'utf8');
...
for (let j = 0; j < groups.length; j++) {
    ...
    if (topic6Str.endsWith(',\n')) {
        topic6Str = topic6Str.slice(0, -2) + '\n';
    }
    topic6Str += `             }${j < groups.length - 1 ? ',' : ''}\n`;
}
topic6Str += `        }\n    },\n`;
...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log('Successfully replaced topic6 with mapped full data!');
} else {
    console.log('Could not find boundaries for topic6');
}
