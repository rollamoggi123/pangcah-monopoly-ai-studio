const fs = require('fs');
const data = require('./topic10_data_mapped.json');
...
const startIdx = content.indexOf('topic10: {');
let endIdx = content.indexOf('topic11: {');
if (startIdx !== -1 && endIdx !== -1) {
    const before = content.substring(0, startIdx);
    const after = content.substring(endIdx);
    content = before + topicStr + '    ' + after;
    fs.writeFileSync('topics.ts', content);
    console.log("Successfully replaced topic10 with mapped full data!");
} else {
    console.log("Could not find topic10 bounds.");
}
